...
def get_token_id(file_name):
    with open(os.path.join(os.getcwd(), file_name), 'r') as file_token:
        token = file_token.readline().strip()
        id_one = file_token.readline().strip()
    return [token, id_one]


def max_size(dict_in_search):
    max_dpi = 0
    need_elem = 0
...
class Vk:
    ...
    def get_photo_info(self):
        ...
        about_photo = requests.get(url, params={**self.start_params, **params}).json()['response']
        return about_photo['count'], about_photo['items']
    ...
    def get_logs_only(self):
        ...
        for i in range(photo_count):
            ...
            url_download, picture_size = max_size(photo_items[i]['sizes'])
...
class Yandex:
    ...
    def in_folder(self, folder_name):
        ...
        for elem in resource:
            folder_list.append(elem['name'])
        return folder_list

    def create_copy(self, my_dict):
        my_folder = self.in_folder(self.folder)
        counter = 0
        for key, i in zip(my_dict.keys(), tqdm(range(self.added_files_num))):
            if counter < self.added_files_num:
                if key not in my_folder:
        ...
        print(f'\nЗапрос завершен, новых файлов загружено: {counter}'
              f'\nВсего файлов в альбоме VK: {len(my_dict)}')
...
if __name__ == '__main__':
    ...
    photo_info = Vk(get_token_id(tokenVK))
    print(photo_info.json)

    with open('my_VK_photo.json', 'w') as outfile:
        json.dump(photo_info.json, outfile)

    my_yandex = Yandex('VK photo copies', get_token_id(tokenYandex), 5)
    my_yandex.create_copy(photo_info.export_dict)
